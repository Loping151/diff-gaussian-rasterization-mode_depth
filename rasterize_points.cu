	...
	const bool debug)
{
  ...
  torch::Tensor out_mode_depth = torch::full({1, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_mode_depth.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, out_mode_depth, radii, geomBuffer, binningBuffer, imgBuffer);
}
